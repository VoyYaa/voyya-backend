...
import { AdminDriverRepository, type CreatedDriverRow } from './admin-driver.repository';
...
export class AdminDriverService {
  ...
  async create(companyId: number, dto: CreateDriverDTO): Promise<CreatedDriver> {
    ...
    try {
      created = await this.prisma.runInTenant(companyId, (tx) =>
        this.repo.createDriverWithVehicle(tx, companyId, {
